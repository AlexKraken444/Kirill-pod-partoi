...
export default function AccountNav() {
  ...
  if (!user) return <a className="accountLink" href="/register">Войти</a>;
  return (
    <div className="accountMenu">
      <span className="avatar" title={user.name}>{user.name.slice(0, 1).toLocaleUpperCase("ru-RU")}</span>
      <button onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); window.location.reload(); }}>Выйти</button>
    </div>
  );
}
