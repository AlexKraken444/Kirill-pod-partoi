...
type Slot = { slot: number; name: string; username: string; verified: boolean; mine: boolean };
  ...
  return (
    <section className="figurineSection" id="figurine" aria-labelledby="figurine-title" data-reveal>
      ...
      <div className="preorderList">
        ...
        <ol>{Array.from({ length: 5 }, (_, index) => { const occupied = state?.slots.find((item) => item.slot === index + 1); return <li className={occupied?.mine ? "mine" : ""} key={index}><span>{String(index + 1).padStart(2, "0")}</span>{occupied ? <a href={`/profile/${encodeURIComponent(occupied.username)}`}>{occupied.name}{occupied.verified && <span className="verifiedBadge" title="Подтверждённый профиль">✓</span>}</a> : <strong>Свободное место</strong>}<i>{occupied ? "ЗАБРОНИРОВАНО" : "СВОБОДНО"}</i></li>; })}</ol>
      </div>
    </section>
  );
}
